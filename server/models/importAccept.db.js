...
export default {
  getIdTotalCount(currentStatus, filters, tenantId) { //获取满足条件的总记录数
      ...
      const sql = `select count(del_no) as count from g_bus_delegate where (tenant_id= ? or rec_tenant_id= ?) ${statusClause} ${filterClause}`;
    ...
    getPagedIdsByCorp(current, pageSize, filters, sortField, sortOrder, currentStatus, tenantId) { //分页显示数据
      ...
      usebook,ems_no,trade_mode, urgent,delegate_type,other_note from g_bus_delegate as T where (T.tenant_id= ? or T.rec_tenant_id= ?) ${statusClause} ${filterClause} ${sortClause}  limit ?, ?`;
    ...
    getStatusCount(tenantId, status, filters) {
      ...
      const filterClause = concatFilterSql(filters, args);
      const sql = `select count(status) as count from g_bus_delegate where (tenant_id= ? or rec_tenant_id= ?) and status in (1,2) and status=? ${filterClause}`;
      console.log(sql, args);
      return mysql.query(sql, args);
    },
    deleteId(idkey) {
