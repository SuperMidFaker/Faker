...
export default {
  getIdTotalCount(currentStatus, filters, tenantId) { //获取满足条件的总记录数
      ...
      const sql = `select count(del_no) as count from g_bus_delegate where (tenant_id= ? or rec_tenant_id= ?) and delegate_type= 1 ${statusClause} ${filterClause}`;
    ...
    getPagedIdsByCorp(current, pageSize, filters, sortField, sortOrder, currentStatus, tenantId) { //分页显示数据
      ...
      usebook,ems_no,trade_mode, urgent,delegate_type,other_note from g_bus_delegate as T where (T.tenant_id= ? or T.rec_tenant_id= ?)  and T.delegate_type= 1  ${statusClause} ${filterClause} ${sortClause}  limit ?, ?`;
    ...
    editExportAccept(entity, key, trans) {
      let updateClause = [];
      let args = [];
