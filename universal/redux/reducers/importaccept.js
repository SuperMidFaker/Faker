...
const domain = '@@welogix/importaccept/';
    ...
    case actionTypes.ID_UPDATE_SUCCEED:
      {
        ...
        idDataArray[state.editIndex] = action.data.importaccept;
        return {...state,
          idlist: {...state.idlist,
            data: idDataArray
          },
    ...
    case actionTypes.IMPORT_EDIT_SUCCEED:
      {
        if (state.selectedIndex !== -1) {
          ...
          idlist.data[state.selectedIndex] = action.data.importaccept;
          return {...state,
            selectedIndex: -1,
            idlist
          };
    ...
    case actionTypes.INVALID_SUCCEED:
      {
        ...
        idlist.data.map((item) => {
          if (item.key !== action.params.acceptId) {
            newlist.push(item);
          }
        });
...
export function updateId(importaccept) {
  return {
    [CLIENT_API]: {
...
export function assignForm(importacceptState, idId) {
  return assignFormC(idId, importacceptState, 'idlist', actionTypes);
}

export function isFormDataLoaded(importacceptState, idId) {
  return isFormDataLoadedC(idId, importacceptState, 'idlist');
}

export function loadForm(cookie, idId) {
  return loadFormC(cookie, 'v1/import/importaccept', {
    pid: idId
  }, actionTypes);
}
...
export function edit(importaccept, params) {
  return {
    [CLIENT_API]: {
...
export function loadLogs(params) {
  return {
    [CLIENT_API]: {
      ...
      endpoint: 'v1/import/importacceptlogs',
      method: 'get',
      params
    }
  };
