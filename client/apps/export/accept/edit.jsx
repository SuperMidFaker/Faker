...
function fetchData({state, dispatch, cookie, params}) {
  ...
  if (pid) {
    ...
    promises.push(dispatch(loadLogs({acceptId: pid, pageSize: state.exportaccept.loglist.pageSize, currentPage: state.exportaccept.loglist.current})));
  } else {
...
@connect(state => ({
  ...
  loglist: state.exportaccept.loglist,
