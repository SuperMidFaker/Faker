...
export default class OverTime extends React.Component {
  ...
  initializeCharts = (props) => {
    ...
    const pieOption = {
      title: { text: '超时占比', textStyle: { fontSize: 14 } },
      tooltip: {
        trigger: 'item',
      ...
      legend: {
        x: 'center',
        bottom: 0,
        data: transitModes.map(item => item.mode_name).concat('准时'),
      },
      color: KPI_CHART_COLORS,
      grid: {
        left: '3%',
  ...
  render() {
    return (<ChartBody loading={this.props.loading} barChartId="bar-chart-overtime" pieChartId="pie-chart-overtime" />);
  }
}
