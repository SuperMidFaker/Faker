...
import { loadOutboundTransactions } from 'common/reducers/cwmTransaction';
...
function fetchData({ state, dispatch }) {
  return dispatch(loadOutboundTransactions({
...
@connect(
  state => ({
    tenantId: state.account.tenantId,
    outboundlist: state.cwmTransaction.list,
...
export default class OutboundTransactionsList extends React.Component {
  ...
  dataSource = new Table.DataSource({
    fetcher: params => this.props.loadOutboundTransactions(params),
  ...
  handleSearch = (value) => {
    ...
    this.props.loadOutboundTransactions({
      tenantId: this.props.tenantId,
      filter: JSON.stringify(filter),
      sorter: JSON.stringify(this.props.sortFilter),
      pageSize: this.props.outboundlist.pageSize,
      current: 1,
    }).then((result) => {
      if (result.error) {
  ...
  render() {
    const { loading, outboundlist } = this.props;
        ...
        <Content className="main-content" key="main">
          <div className="page-body">
            <div className="toolbar">
              <SearchBar placeholder={this.msg('outboundListSearchPlaceholder')} size="large" onInputSearch={this.handleSearch} />
