    ...
    render: (o, record) => {
      ...
      if (outboundHead.shipping_mode === 'manual') {
        switch (record.status) {  // 分配明细的状态 2 已分配 4 已拣货 6 已发运
          case 2:   // 已分配
            return (<span>
              <RowUpdater onClick={() => this.handleConfirmPicked(record.id, record.location, record.alloc_qty, record.sku_pack_qty, record.trace_id)} label="拣货确认" row={record} />
              <span className="ant-divider" />
              <RowUpdater onClick={this.handleCancelAllocated} label="取消分配" row={record} />
            </span>);
          case 3:   // 部分拣货
            return (
              <span>
                <RowUpdater onClick={() => this.handleConfirmPicked(record.id, record.location, record.alloc_qty, record.sku_pack_qty, record.trace_id)} label="拣货确认" row={record} />
                <span className="ant-divider" />
                <RowUpdater onClick={() => this.handleCancelPicked(record.id, record.picked_qty, record.picked_qty / record.sku_pack_qty)} label="取消拣货" row={record} />
              </span>
            );
          case 4:   // 已拣货
            return (<span>
              <RowUpdater onClick={() => this.handleConfirmShipped(record.id, record.picked_qty, record.sku_pack_qty)} label="发货确认" row={record} />
              <span className="ant-divider" />
              <RowUpdater onClick={() => this.handleCancelPicked(record.id, record.picked_qty, record.picked_qty / record.sku_pack_qty)} label="取消拣货" row={record} />
            </span>);
          case 5:   // 已复核装箱
