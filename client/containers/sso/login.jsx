  ...
  render() {
    ...
    return (
      <div className="panel-body">
        { error ? <div>{
          `C${error.code}: ${(error.message ? formatMsg(intl, error.message.key, error.message.values) : error.message)}`
        }</div> : null }
        <form onSubmit={::this.handleSubmit} className="form-horizontal">
          <div className="login-form">
            <div className="form-group">
              <div className="input-group"><span className="input-group-addon"><i className="icon s7-user"></i></span>
                <input ref="username" type="text" placeholder={formatMsg(intl, 'userPlaceholder')} autoComplete="off" className="form-control" value={username}
            ...
            <div className="form-group">
              <div className="input-group"><span className="input-group-addon"><i className="icon s7-lock"></i></span>
                <input ref="password" type="password" placeholder={formatMsg(intl, 'pwdPlaceholder')} className="form-control"
                onChange={(ev) => this.handleTextChange(ev, 'password')}/>
              </div>
            </div>
            <div className="form-group login-submit">
              <button data-dismiss="modal" type="submit" className="btn btn-primary btn-lg">{formatMsg(intl, 'login')}</button>
            </div>
            <div className="form-group footer row">
              <div className="col-xs-6"><NavLink to="/forgot">{formatMsg(intl, 'forgotPwd')}?</NavLink></div>
