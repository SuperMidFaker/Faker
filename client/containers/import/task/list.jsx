...
import { loadStatus,loadTask, loadTenantsByMaster, delTask, switchTenant, switchStatus } from
...
export default class TaskSetting extends React.Component {
  ...
  handleChangeStatus(type, status) {
    ...
    this.setState({
      ...
      statusToMe:"statusToMe" === type?'primary':'ghost',
  ...
  render() {
    const { branches, tenant, statusList: {toMeCount,notSendCount, notAcceptCount, acceptCount, invalidCount}, tasklist, loading, needUpdate } = this.props;
    const { statusToMe, statusAll, statusAccept, statusInvalid, statusNotAccept, statusNotSend, currentStatus } = this.state;
    ...
    <Button type={statusToMe} style={{marginRight:5}} onClick={ () => this.handleChangeStatus('statusToMe', -1) } ><span>分配给我({toMeCount})</span></Button>
    <Button type={statusNotSend} style={{marginRight:5}} onClick={ () => this.handleChangeStatus('statusNotSend', 0) } ><span>未申报({notSendCount})</span></Button>
    <Button type={statusNotAccept} style={{marginRight:5}} onClick={ () => this.handleChangeStatus('statusNotAccept', 1) } ><span>已申报({notAcceptCount})</span></Button>
    <Button type={statusAccept} style={{marginRight:5}} onClick={ () => this.handleChangeStatus('statusAccept', 2) } ><span>已放行({acceptCount})</span></Button>
    <Button type={statusInvalid} style={{marginRight:5}} onClick={ () => this.handleChangeStatus('statusInvalid', 4) } ><span>已结单({invalidCount})</span></Button>
    <div className="pull-right action-btns">
            </div>
          </div>
