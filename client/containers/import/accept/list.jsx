...
@connect(state => ({ // 从初始化state中加载数据
  tenantId: state.account.tenantId,
  idlist: state.importaccept.idlist,
  sendlist: state.importaccept.sendlist,
  statusList: state.importaccept.statusList,
...
export default class ImportAccept extends React.Component {
  ...
  renderColumnText(record, text, ishref = false) {
    ...
    if (ishref === true) {
      return <NavLink to={`/import/accept/edit/${record.key}`}>{text}</NavLink>;
    } else {
