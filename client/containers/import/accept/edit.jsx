...
'../../../../universal/redux/reducers/importaccept';
...
function fetchData({state, dispatch, cookie, params}) {
  ...
  if (pid) {
    if (!isFormDataLoaded(state.importaccept, pid)) {
      promises.push(dispatch(loadForm(cookie, pid)));
    } else {
      promises.push(dispatch(assignForm(state.importaccept, pid)));
    }
    promises.push(dispatch(loadLogs({acceptId: pid, pageSize: state.importaccept.loglist.pageSize, currentPage: state.importaccept.loglist.current})));
  } else {
...
@connect(state => ({
  selectedIndex: state.importaccept.selectedIndex,
  loglist: state.importaccept.loglist,
  ...
  selectOptions: state.importaccept.selectOptions,
...
export default class ImportAcceptEdit extends React.Component {
  static propTypes = {
  ...
  handleConfirmOk() {
    ...
    this.props.invalidAccept({
      tenantId: this.props.tenantId,
      loginId: this.props.loginId,
      username: this.props.username,
      acceptId: this.props.formData.key,
